...
const WineCategoryData = [
  { name: 'Alice', id: 1, slug: 'alice' },
  { name: 'Bob', id: 2, slug: 'bob' },
  { name: 'Charlie', id: 3, slug: 'charlie' },
  { name: 'Diana', id: 4, slug: 'diana' },
  { name: 'Eve', id: 5, slug: 'eve' }
];
...
const WineHardwareComponent = () => {
  const [selectedCategory, setSelectedCategory] = useState('alice');
  const [selectedHardwareName, setSelectedHardwareName] = useState('Alice');
      ...
      >
        <Box sx={{ display: "flex", gap: "12px", }}>
          ...
          >
            <Select
              ...
              displayEmpty
              renderValue={(value) => {
                const selectedItem = WineCategoryData?.find(item => item?.slug === value);
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {false ? (
            ...
            hardwareWineData && hardwareWineData?.length <= 0 ? (
              <Typography sx={{ color: "#667085", p: 2, textAlign: "center", background: '#FFFF' }}>
                No Hardware Found
              </Typography>
            ) : hardwareWineData.map((data, index) => (
              <WineHardwareTable data={data} key={index} selectedSlug={selectedCategory} />
            ))
          }
        </Box>
      </Box>
