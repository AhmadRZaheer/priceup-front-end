...
const MirrorsEdgeWorkComponent = () => {
  ...
  //                 : option.cost,
  //     }));
  //     editGlassType({ data: { options: updatedOptions }, apiRoute: `${routePrefix}/${data._id}` });
  // };
  const handleUpdateCost = (data) => {
    handleClose();
    const updatedOptions = data.options.map((option) => ({
          ...
          <>
            <Box
              sx={{
                // height: "21px",
                bgcolor:
                  params?.row?.options[1]?.status === true
                    ? "#EFECFF"
                    : "#F3F5F6",
                borderRadius: "70px",
                p: "6px 8px",
